...
else
{
  ...
  const RatingComponent = (props) => {
    const {precision,size,name,defaultValue,disabled,readOnly,hoverFeedback,max,labels,icon,emptyIcon,color,iconstar} = props;
   ...
   //const customiconstyle=getcustomratingcolor(ratingColor,ratingHoverColor);
    ...
    >
      ...
      size={size}
        ...
        emptyIcon={<Icon style={{ opacity: 0.55 }} fontSize="inherit">{iconstar}</Icon>}

        sx={<Icon fontSize="inherit">{iconstar}</Icon> && customiconstyle}

        icon={<Icon fontSize="inherit">{iconstar}</Icon>}
